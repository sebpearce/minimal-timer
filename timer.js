...
(function () {
  ...
  var convertToDigits = function convertToDigits(string) {
  ...
  var getTimeURLQuery = function getTimeURLQuery() {
  ...
  var startTimer = function startTimer(totalSeconds) {
  ...
  var unpauseTimer = function unpauseTimer() {
  ...
  var calcTime = function calcTime(seconds) {
  ...
  var leadingZero = function leadingZero(time) {

    return time < 10 ? "0" + time : +time;
  };

  var convertUserInput = function convertUserInput(string) {
    ...
    if (!ok || isNaN(result) || result <= 0) return 0;else {
      if (result > 86400) {
